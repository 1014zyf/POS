...
public class PosOctopusTransaction {
	...
	private void readDataFromJson(JSONObject transactionsJSONObject) {
		...
		this.init();
		this.octtId = resultOctopusTransaction.optString("octt_id");
		this.bdayId = resultOctopusTransaction.optString("octt_bday_id");
		this.shopId = resultOctopusTransaction.optInt("octt_shop_id");
		this.oletId = resultOctopusTransaction.optInt("octt_olet_id");
		this.chksId = resultOctopusTransaction.optString("octt_chks_id");
		this.cpayId = resultOctopusTransaction.optString("octt_cpay_id");
		...
		if(!sLocTime.isEmpty())
			...
			this.transactionLocTime = AppGlobal.getCurrentTime(false);
		this.transactionTime = resultOctopusTransaction.optString("octt_transaction_time", oFmt.print(AppGlobal.convertTimeToUTC(transactionLocTime)));
	...
	public boolean printOctopusSlip(boolean bDuplicate, int iOutletId, String sDeviceId, String sUdsn, String sCardId, String sCardType, BigDecimal dTransAmount, BigDecimal dRemainAmount, int iPrintQueueId, int iLangIndex, int iTransactionNum, String sLastAddValueType, String sLastAddValueDate, String sTransactionTime) {
		JSONObject requestJSONObject = new JSONObject();
		boolean bResult = false;
		...
		try {
			...
			requestJSONObject.put("cardType", sCardType);
	...
	public void init() {
		...
		this.transactionLocTime = AppGlobal.getCurrentTime(false);
		this.transactionTime = fmt.print(AppGlobal.convertTimeToUTC(transactionLocTime));
		this.transactionUserId = 0;
		this.transactionStatId = 0;
		this.status = PosOctopusTransaction.STATUS_ACTIVE;
	}
	
	public String getOcttId() {
		return this.octtId;
	}
	...
	public void setOcttId(String sOcttId) {
		this.octtId = sOcttId;
	}
