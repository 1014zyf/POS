...
public class PosOutletItem {
	...
	//get item list
	public JSONArray readAllBySoldoutAndShop(int iShopId, int iOutletId, String sSoldout) {
		JSONObject requestJSONObject = new JSONObject();
		JSONArray responseJSONArray = null;
		...
		responseJSONArray = this.readDataListFromApi("gm", "pos", "getAllPosOutletItemsBySoldoutAndShopId", requestJSONObject.toString());
	...
	public boolean addUpdateWithMultipleRecord(ArrayList<PosOutletItem> oPosOutletItems) {
	...
	//add or update multiple outlet items record to database
	public boolean addUpdateWithMultipleShopRecord(int iShopId, ArrayList<PosOutletItem> oPosOutletItems) {
		JSONObject tempOutletItemJSONObject = null, outletItemJSONObject = new JSONObject();
		JSONArray outletItemJSONArray = new JSONArray();
		...
		try {
			outletItemJSONObject.put("outletItems", outletItemJSONArray);
			outletItemJSONObject.put("shopId", iShopId);
		}catch (JSONException jsone) {
			jsone.printStackTrace();
		}
		
		if (!OmWsClientGlobal.g_oWsClient.get().call("gm", "pos", "saveMultiShopOutletItems", outletItemJSONObject.toString(), false))
			return false;
		else
			return true;
	}
	...
	public JSONArray addUpdateWithMultipleRecordWithMenuAndItemIdsAndShopId(int iShopId, int iOutletId, List<Integer> oMenuIds, List<Integer> oItemIds, String sSoldout) {
		JSONObject requestJSONObject = new JSONObject();
		JSONArray responseJSONArray = null;
		JSONArray oMenuIdArray = new JSONArray(), oItemIdArray = new JSONArray();
