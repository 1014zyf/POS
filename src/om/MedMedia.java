...
public class MedMedia {
	...
	//read record and media url by media file name
	public boolean readInfoAndUrlByFileName(String sFileName) {
		JSONObject requestJSONObject = new JSONObject();
		
		try{
			requestJSONObject.put("fileName", sFileName);
		}catch (JSONException jsone) {
			jsone.printStackTrace();
		}
		
		return this.readDataFromApi("gm", "media", "getMediaAndUrlByFileName", requestJSONObject.toString());
