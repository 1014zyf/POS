...
public class PosBusinessDay{
	...
	private OutCalendar holidayCalendar;
	private OutCalendar beforeHolidayCalendar;
	private OutCalendar specialDayCalendar;
	private OutCalendar beforeSpecialDayCalendar;
	...
	public PosBusinessDay(int iOletId, int iShopId, int iStartUserId) {
		...
		this.startTime = fmt.print(AppGlobal.convertTimeToUTC(today));
	...
	protected JSONObject constructAddSaveJSON(boolean bUpdate) {
		...
		try {
			...
			if (this.date != null)
				addSaveJSONObject.put("bday_date", fmt.print(this.date));
			...
			addSaveJSONObject.put("bday_gratuity_round", this.gratuityRound);
			...
			addSaveJSONObject.put("bday_gratuity_decimal", this.gratuityDecimal);
	...
	private void readDataFromJson(JSONObject businessDayJSONObject) {
		...
		this.init();
		this.bdayId = resultBusinessDay.optString("bday_id");
		...
		this.gratuityRound = resultBusinessDay.optString("bday_gratuity_round");
		...
		this.gratuityDecimal = resultBusinessDay.optInt("bday_gratuity_decimal");
		for(i=1; i<=30; i++)
		...
		if(holidayCaldId > 0 && businessDayJSONObject.has("holidayCalendar"))
			holidayCalendar = new OutCalendar(businessDayJSONObject.optJSONObject("holidayCalendar"));
		
		if(dayBeforeHolidayCaldId > 0 && businessDayJSONObject.has("beforeHolidayCalendar"))
			beforeHolidayCalendar = new OutCalendar(businessDayJSONObject.optJSONObject("beforeHolidayCalendar"));
		
		if(specialDayCaldId > 0 && businessDayJSONObject.has("specialDayCalendar"))
			specialDayCalendar = new OutCalendar(businessDayJSONObject.optJSONObject("specialDayCalendar"));
		
		if(dayBeforeSpecialDayCaldId > 0 && businessDayJSONObject.has("beforeSpecialDayCalendar"))
			beforeSpecialDayCalendar = new OutCalendar(businessDayJSONObject.optJSONObject("beforeSpecialDayCalendar"));
	}
	...
	public void init() {
		...
		beforeHolidayCalendar = null;
		specialDayCalendar = null;
		beforeSpecialDayCalendar = null;
		m_sLastErrorMessage = "";
	}
	
	//read the object form database by bday_id
	public boolean readById(String sBdayId, String sStatus) {
		JSONObject requestJSONObject = new JSONObject();
		
		try {
			requestJSONObject.put("id", sBdayId);
	...
	//read the object form database by bday_olet_id
	public int dailyStart(int iOutletId, int iUserId, boolean bRecallLastBusinessDay) {
		JSONObject requestJSONObject = new JSONObject();
		...
		if (!OmWsClientGlobal.g_oWsClient.get().call("gm", "pos", "dailyStart", requestJSONObject.toString(), false)) {
			...
			// For carry forward checks
			if(OmWsClientGlobal.g_oWsClient.get().getResponse().has("checks"))
				return OmWsClientGlobal.g_oWsClient.get().getResponse().optInt("checks");
	...
	//read the object form database by bday_olet_id
	public boolean reloadBusinessSetting(String sBusinessDayId, int iUserId) {
		JSONObject requestJSONObject = new JSONObject();
	...
	//read the object form database by bday_shop_id, bday_olet_id, a period of time
	public JSONArray readListByShopOrOutletWithPeriodOfTime(int iShopId, int iOutletId, String sStartDate, String sEndDate) {
		JSONObject requestJSONObject = new JSONObject();
		...
		responseJSONArray = this.readDataListFromApi("gm", "pos", "getBusinessDaysByShopOutletAndTimeOfPeriod", requestJSONObject.toString());
		
		return responseJSONArray;
	}
	
	// Purge transaction data
	// *** This function can only work on standalone account
	public boolean purgeTransactionData(String sBusinessDayId) {
		JSONObject requestJSONObject = new JSONObject();
		...
		if (!OmWsClientGlobal.g_oWsClient.get().call("gm", "pos", "purgeTransactionData", requestJSONObject.toString(), false)) {
			m_sLastErrorMessage = OmWsClientGlobal.g_oWsClient.get().getLastErrorMessage();
			return false;
		}else {
			return true;
	...
	public void setGratuityRound(String sGratuityRound) {
		this.gratuityRound = sGratuityRound;
	}
	...
	public void setGratuityDecimal(int iGratuityDecimal) {
		this.gratuityDecimal = iGratuityDecimal;
	}
	...
	public String getDateInString() {
		DateTimeFormatter dateFormat = DateTimeFormat.forPattern("yyyy-MM-dd");
		return dateFormat.print(this.date);
	}
	...
	public String getDateInStringWithFormat(String sStringFormat) {
		...
		DateTimeFormatter dateFormat = DateTimeFormat.forPattern(sStringFormat);
		return dateFormat.print(this.date);
	}
	...
	public String[] getBeforeHolidayCalendarName() {
		if(beforeHolidayCalendar == null)
			return null;
		else
			return beforeHolidayCalendar.getName();
	}
	
	public String[] getSpecialDayCalendarName() {
		if(specialDayCalendar == null)
			return null;
		else
			return specialDayCalendar.getName();
	}
	
	public String[] getBeforeSpecialDayCalendarName() {
		if(beforeSpecialDayCalendar == null)
			return null;
		else
			return beforeSpecialDayCalendar.getName();
	}
}
