...
public class OutShop {
	...
	public OutShop(JSONObject shopJSONObject) {
		this.readDataFromJson(shopJSONObject);
	}
	...
	protected JSONObject constructAddSaveJSON(boolean bUpdate) {
		...
		try {
			...
			if(!this.timezoneName.isEmpty())
				addSaveJSONObject.put("shop_timezone_name", this.timezoneName);
	...
	private void readDataFromJson(JSONObject shopJSONObject) {
		...
		this.timezoneName = resultShop.optString("shop_timezone_name", "");
