...
public class PosTaiwanGuiTran {
	...
	//					Empty - no error
	public String addUpdate(boolean bUpdate, int iTargetTransNum, int iChkId) {
		JSONObject requestJSONObject = new JSONObject();
		
		requestJSONObject = constructAddSaveJSON(bUpdate);
		try {
			requestJSONObject.put("skipTransNum", iTargetTransNum);
			requestJSONObject.put("checkId", iChkId);
		...
		if (!OmWsClientGlobal.g_oWsClient.get().call("gm", "pos", "saveTaiwanGuiTran", requestJSONObject.toString(), false))
			...
			if (this.twtxId == 0) {
				...
				this.twtxId = OmWsClientGlobal.g_oWsClient.get().getResponse().optInt("id");
			}
	...
	//get last taiwan gui record by config id and station id
	public boolean getLastTaiwanGuiTranByConfigAndStation(int iConfigId, int iStatId) {
		JSONObject requestJSONObject = new JSONObject();
		...
		return this.readDataFromApi("gm", "pos", "getLastTaiwanGuiTranByStation", requestJSONObject.toString());
	}
