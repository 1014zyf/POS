...
public class ResvResv {
	...
	private void readDataFromJson(JSONObject resvJSONObject) {
		...
		this.init();
		this.resvId = resultResv.optInt("resv_id");
		...
		if (!sBookDate.isEmpty()) {
			try {
				this.bookDate = new Date(dateFormat.parse(sBookDate).getTime());
		...
		if (!sExpiryDate.isEmpty()) {
			try {
				this.expiryDate = new Date(dateFormat.parse(sExpiryDate).getTime());
			} catch (ParseException e) {
				e.printStackTrace();
			}
		}
