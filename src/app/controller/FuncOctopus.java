...
public class FuncOctopus {
	...
	public void addOctopusTransaction(int iBdayId, int iShopId, int iOletId, int iChksId, int iCpayId, String sTransactionType, String sTransactionTime, BigDecimal dTransactionAmount, int iPaymId, String sDeviceId, String sUdsn,
		...
		oPosOctopusTransaction.setTransactionTime(fmt.print(oCurrentDateTime.withZone(DateTimeZone.UTC)));
		
		oPosOctopusTransaction.setBdayId(iBdayId);
		oPosOctopusTransaction.setShopId(iShopId);
		oPosOctopusTransaction.setOletId(iOletId);
		oPosOctopusTransaction.setChksId(iChksId);
		oPosOctopusTransaction.setCpayId(iCpayId);
		...
		if(sTransactionType.equals(PosOctopusTransaction.TYPE_ADD_VALUE)){
			...
			for(int i=1; i<=m_iOctopusAddValueSlipCopiesCount; i++){
				Object[] oParameter1s = new Object[13];
				if(i == 1)
				...
				oParameter1s[5] = dTransactionAmount;
				oParameter1s[6] = dCardCurrentAmount;
				oParameter1s[7] = AppGlobal.g_oFuncStation.get().getCheckPrtqId();
				oParameter1s[8] = AppGlobal.g_oCurrentLangIndex.get();
				oParameter1s[9] = m_iNextStationTransactionNumber;
				oParameter1s[10] = sLastAddValueType;
				oParameter1s[11] = sLastAddValueDate;
				oParameter1s[12] = sTransactionTime;
