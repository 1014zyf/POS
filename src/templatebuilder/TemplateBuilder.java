...
public class TemplateBuilder {
	...
	public void setupBasicElement(VirtualUIBasicElement oObject, Element node) {
		...
		oObject.setTextAlign("");
		if(node.getAttributeValue("textAlign") != null) {
			oObject.setTextAlign(node.getAttributeValue("textAlign"));
		}
		oObject.setShadowTop(0);
		if(node.getAttributeValue("shadowTop") != null) {
			oObject.setShadowTop(Integer.parseInt(node.getAttributeValue("shadowTop")));
		}
		oObject.setShadowLeft(0);
		if(node.getAttributeValue("shadowLeft") != null) {
			oObject.setShadowLeft(Integer.parseInt(node.getAttributeValue("shadowLeft")));
		}
		oObject.setShadowRadius(0);
		if(node.getAttributeValue("shadowRadius") != null) {
			oObject.setShadowRadius(Integer.parseInt(node.getAttributeValue("shadowRadius")));
		}
		oObject.setShadowColor("");
		if(node.getAttributeValue("shadowColor") != null) {
			oObject.setShadowColor(node.getAttributeValue("shadowColor"));
		}
	}
	...
	public boolean buildCodeReader(VirtualUICodeReader oFrame, String sFrameName){
		if (m_oRootElement == null)
			return false;
		
		Elements elements = m_oRootElement.getChildElements("CODEREADER");
		for(int m = 0; m < elements.size(); m++) {
			Element node = elements.get(m);
			if(node.getAttributeValue("class").equals(sFrameName)) {
				this.setupBasicElement(oFrame, node);
	...
	public boolean buildCanvas(VirtualUICanvas oCanvas, String sCanvasName) {
		if (m_oRootElement == null)
			return false;
		Elements elements = m_oRootElement.getChildElements("CANVAS");
		for(int m = 0; m < elements.size(); m++) {
			Element node = elements.get(m);
			if(node.getAttributeValue("class").equals(sCanvasName)) {
				this.setupBasicElement(oCanvas, node);
				
				oCanvas.setTextSize(0);
				if(node.getAttributeValue("textSize") != null) {
					oCanvas.setTextSize(Integer.parseInt(node.getAttributeValue("textSize")));
				}
				oCanvas.setCornerRadius("");
				if(node.getAttributeValue("cornerRadius") != null) {
					oCanvas.setCornerRadius(node.getAttributeValue("cornerRadius"));
				}
				// quit looping if result found
				return true;
			}
